/*------------------------------------------------------------------------*/
/*  File pd21316c.cpp                                                     */
/*  Model PCDitch v2.13.16 with changes M version                         */
...
{   cout << endl << "Analyser main" << endl;
    //
    int returnValue;
    pcditch = new PCModel(a.getSimLngId(),
...
{   cout << endl << "Construct system" << endl;
    s.setNewSysTask(s.getAna().getSimDblTime(), PCDitch21316StartSys);
    // start a new task that builds up the system at the time specified in the first dbl column of the analyser table
    s.setNewSysTask(365.0 * s.getAna().getDbl(ANA_DBL_REP_START), PCDitch21316RepSys);
    // start a new task for reporting at the time specified in the first dbl column of the analyser table
    s.setNewSysTask(365.0 * s.getAna().getDbl(ANA_DBL_AVG_START), PCDitch21316ColAvgSys);
    // start a new task for averaging at the time specified in the 'start averaging' column of the analyser table
    s.setNewSysTask(365.0 * s.getAna().getDbl(ANA_DBL_AVG_START) + 365.0, PCDitch21316RepAvgSys);
    // start a new task for averaging at the time specified in the 'start averaging' column of the analyser table
    s.setNewSysTask(365.0 * s.getAna().getAnaDblReady() + 0.0000001, PCDitch21316StopSys);
    // start a new task ending the simulation at  the time specified in the second dbl column of the analyser table
    s.setNewSysTask(s.getAna().getSimDblTime() + 0.0000002, PCDitch21316PerformOneIntegrationStep);
...
{    Analyser &a = Analyser::getAna();
     ...
     System &s = a.getSys();
     //
     SystemTask &t = s.setNewSysTask(startTime, PCDitch21316ReadParam);
     //
     t.setNumber(id);
     //
     return t.getTaskId();
...
{    Analyser &a = Analyser::getAna();
     ...
     System &s = a.getSys();
     //
     SystemTask &t = s.setNewSysTask(startTime, PCDitch21316CompData);
     //
     t.setNumber(id);
     //
     return t.getTaskId();
...
{   double taskTime = t.getTime();
    ...
    // get a reference to the analyser object
//*************************SWITCH TO PCMODEL**********************************//
    pcditch->colAvgSys(taskTime, a.getDbl(ANA_DBL_AVG_START_WITHIN_YEAR), a.getDbl(ANA_DBL_AVG_END_WITHIN_YEAR));
    // collect the averages of the time dependent output
//*************************SWITCH TO PCMODEL**********************************//
...
{   System &s = t.getSys();
    ...
    double taskTime = t.getTime();
    // get the current time
    pcditch->performOneIntegrationStep(taskTime);
...
{   double taskTime = t.getTime();
    ...
    pcditch->compareWithData(taskTime, nextTaskTime, number, stopTask);
...
{   System &s = t.getSys();
    ...
    // declare a logical variable that determines if this task is to be performed again, this value is set by the compData function
//*************************SWITCH TO PCMODEL**********************************//
    pcditch->readForcingFunction(taskTime, nextTaskTime, number, stopTask);
//*************************SWITCH TO PCMODEL**********************************//
    // task to handle the forcing functions, including interpolation
    if (stopTask) t.setStopTask();
