...
_uDepthWEpi_ =_IF_ ( _aInclStrat_ _EQ_ _TRUE_ ) _THEN_ _uDepthMix_ _ELSE_ _sDepthW_ _ENDIF_;
_uDepthWHyp_ =_IF_ ( _aInclStrat_ _EQ_ _TRUE_ ) _THEN_ _sDepthW_ - _uDepthMix_ _ELSE_ _cMinDepthHypEpi_ _ENDIF_;
...
_uVegHeight_ =  _IF_ ( _UseVegHeight_ _EQ_ _FALSE_ _AND_  _sVegHeight_ _LT_ _sDepthW_ ) _THEN_ _sVegHeight_ _ELSEIF_ ( _UseVegHeight_ _EQ_ _FALSE_ ) _THEN_ ( _sDepthW_ ) _ELSE_ _MIN_ ( _sVegHeight_ , _sDepthW_ , _POW_ ( _cVegHPar1_*_sDVeg_ , _cVegHPar2_ ) ) _ENDIF_;
_aVegHeight_ =   _POW_ ( _cVegHPar1_*_sDVeg_ , _cVegHPar2_ ) ;
...
_uVegHeightLightSum_ = _cVegL_ *_LN_( 1.0 + _MAX_ (_uVegHeight_, _MIN_ (_sDepthW_, _cVegHeight_) ) );
...
_aLPARBotEpiOpen_ = _uLPARSurf_ * _EXP_(- _aExtCoefOpenEpi_ * _uDepthWEpi_);
...
_aExtVegHyp_ = _cExtSpVeg_ * _aDSubVeg_ / _uDepthWHyp_ * (_uVegHeightHyp_ /_sDepthW_ );
_aExtCoefHyp_ = _aExtCoefOpenHyp_ + _aExtVegHyp_;
_aExtVegEpi_ = _cExtSpVeg_ * _aDSubVeg_ / _uDepthWEpi_ * (_uVegHeightEpi_ /_sDepthW_ ) ;
_aExtCoefEpi_ = _aExtCoefOpenEpi_ + _aExtVegEpi_;
_aLPARBotEpi_ = _uLPARSurf_ * _EXP_(- _aExtCoefEpi_ * _uDepthWEpi_);
...
_uQOutEpi_ = _IF_ (_ReadQOut_ _EQ_ _TRUE_) _THEN_ _MAX_(_mQOut_, (_uDepthWEpi_ - _cDepthWMax_) * _cMaxWOut_ * _mmPerm_) _ELSEIF_ (_FALSE_ _EQ_ _aInclStrat_) _THEN_ _MAX_(0.0, (_uQIn_ - _uQInExtra_) - _uQEv_ - _cQInf_ ) + _uQOutExtra_ _ELSE_ _MAX_(0.0, (_uQIn_ - _uQInExtra_) - _uQEv_ - _cQInf_ ) + _uQOutExtra_ _ENDIF_;
_uQOutHyp_ = 0.0;
...
_uTauWat_ = _IF_ ( _cQInf_ _GT_ 0.0 ) _THEN_ 1.0 / (_uQIn_ / _mmPerm_ / _sDepthW_ +_NearZero_) _ELSE_ 1.0 / ((_uQIn_ - _cQInf_ ) / _mmPerm_ / _sDepthW_ +_NearZero_) _ENDIF_;
...
_vTranHypEpiW_ = _IF_ (_InclStrat_ _EQ_ _FALSE_) _THEN_ 0.0 _ELSEIF_ ( _sDepthW_ -_uDepthMix_ _LT_ _cMinDepthHypEpi_ ) _THEN_ ( _sDepthW_ - _cMinDepthHypEpi_ -_uDepthWEpi_ ) * _kExchW_ _ELSEIF_ ( _uDepthMix_ _LT_ _cMinDepthHypEpi_ ) _THEN_ ( _cMinDepthHypEpi_ -_uDepthWEpi_ ) * _kExchW_ _ELSE_ ( _uDepthMix_ -_uDepthWEpi_ ) * _kExchW_ _ENDIF_;
...
_uExchMaxW_ =   _IF_ ( _aInclStrat_ _EQ_ _TRUE_) _THEN_ _kExchMaxW_ /( _aStrat_* _fMeta_ * _sDepthW_ + _cDepthMetaMin_ ) _ELSE_ 0.0 _ENDIF_;
...
_aFunDimSusp_ = _IF_ (_uTmEpi_ _GE_ 0.1) _OR_  _POW_(((_cFetch_ +_NearZero_)/ _cFetchRef_) , (0.5)) _LE_ _NearZero_ _THEN_ _cSuspRef_ * ((_cSuspMin_ + _cSuspMax_ / (1.0 + _EXP_(_cSuspSlope_ * (_sDepthW_ - _hDepthSusp_)))) * _POW_(((_cFetch_ +_NearZero_)/ _cFetchRef_) , (0.5))) _ELSE_ 0.0 _ENDIF_;
...
_bfSubVegEpi_ = _bfSubVeg_ * ( _uVegHeightEpi_ / _sDepthW_ );
_bfSubVegHyp_ = _bfSubVeg_ * ( _uVegHeightHyp_ / _sDepthW_ );
...
_wDPrimBlueWHyp_ = _IF_ (_aInclStrat_ _EQ_ _FALSE_) _THEN_ 0.0 _ELSE_ _wDAssBlueHyp_ - _wDRespBlueWHyp_ - _wDLossBlueHyp_ - _wDMortBlueWHyp_ -(_tDSetBlueHyp_ - _tDSetBlueEpi_ - _tDResusBlue_) / _uDepthWHyp_ _ENDIF_;
_wPPrimBlueWHyp_ = _IF_ (_aInclStrat_ _EQ_ _FALSE_) _THEN_ 0.0 _ELSE_ _wPUptBlueHyp_ - _wPExcrBlueWHyp_ - _wPLossBlueHyp_ - _wPMortBlueWHyp_ -(_tPSetBlueHyp_ - _tPSetBlueEpi_ - _tPResusBlue_) / _uDepthWHyp_ _ENDIF_;
_wNPrimBlueWHyp_ = _IF_ (_aInclStrat_ _EQ_ _FALSE_) _THEN_ 0.0 _ELSE_ _wNUptBlueHyp_ - _wNExcrBlueWHyp_ - _wNLossBlueHyp_ - _wNMortBlueWHyp_ -(_tNSetBlueHyp_ - _tNSetBlueEpi_ - _tNResusBlue_) / _uDepthWHyp_ _ENDIF_;
...
_aTransparencyT_ = _IF_ ( _aInclStrat_ _EQ_ _TRUE_ ) _THEN_ _aSecchiT_ / _sDepthW_ _ELSE_ _aSecchiT_ / _uDepthWHyp_ _ENDIF_;
...
_afVolMarshEpi_ = _IF_ (_InclMarsh_ _EQ_ _TRUE_ _AND_ _fMarsh_ _GT_ _NearZero_) _THEN_ _fMarsh_ * _sDepthWM_ / _uDepthWEpi_ _ELSE_ 0.0 _ENDIF_;
...
_tO2AerEpi_ = _kAer_ * _uFunTmAerEpi_ * (_uO2SatEpi_ - _oO2WEpi_) * _aFunFloatAer_;
...
_aLLimBlueEpi_ = _UseSteeleBlue_ *(_EXP_(1.0) /(_aExtCoefEpi_ * _uDepthWEpi_) *(_EXP_(- _aLPARBotEpi_ /(_cLOptRefBlue_ * _uFunTmProdBlueEpi_)) - _EXP_(- _uLPARSurf_ /(_cLOptRefBlue_ * _uFunTmProdBlueEpi_)))) +(1.0 - _UseSteeleBlue_) *(1.0 /(_aExtCoefEpi_ * _uDepthWEpi_) * _LN_((1.0 + _uLPARSurf_ / (_hLRefBlue_ * _uFunTmProdBlueEpi_)) / (1.0 + _aLPARBotEpi_ /(_hLRefBlue_ * _uFunTmProdBlueEpi_))));
...
_wDPrimBlueWEpi_ = _IF_ (_aInclStrat_ _EQ_ _TRUE_ ) _THEN_ _wDAssBlueEpi_ - _wDRespBlueWEpi_ - _wDLossBlueEpi_ - _wDMortBlueWEpi_ -(_tDSetBlueEpi_ ) / _uDepthWEpi_ _ELSE_ _wDAssBlueEpi_ - _wDRespBlueWEpi_ - _wDLossBlueEpi_ - _wDMortBlueWEpi_ -(_tDSetBlueEpi_ - _tDResusBlue_) / _uDepthWEpi_ _ENDIF_;
_wPPrimBlueWEpi_ = _IF_ (_aInclStrat_ _EQ_ _TRUE_ ) _THEN_ _wPUptBlueEpi_ - _wPExcrBlueWEpi_ - _wPLossBlueEpi_ - _wPMortBlueWEpi_ -(_tPSetBlueEpi_ ) / _uDepthWEpi_ _ELSE_ _wPUptBlueEpi_ - _wPExcrBlueWEpi_ - _wPLossBlueEpi_ - _wPMortBlueWEpi_ -(_tPSetBlueEpi_ - _tPResusBlue_) / _uDepthWEpi_ _ENDIF_;
_wNPrimBlueWEpi_ = _IF_ (_aInclStrat_ _EQ_ _TRUE_ ) _THEN_ _wNUptBlueEpi_ - _wNExcrBlueWEpi_ - _wNLossBlueEpi_ - _wNMortBlueWEpi_ -(_tNSetBlueEpi_ ) / _uDepthWEpi_ _ELSE_ _wNUptBlueEpi_ - _wNExcrBlueWEpi_ - _wNLossBlueEpi_ - _wNMortBlueWEpi_ -(_tNSetBlueEpi_ - _tNResusBlue_) / _uDepthWEpi_ _ENDIF_;
...
_aLLimGrenEpi_ = _UseSteeleGren_ *(_EXP_(1.0) /(_aExtCoefEpi_ * _uDepthWEpi_) *(_EXP_(- _aLPARBotEpi_ /(_cLOptRefGren_ * _uFunTmProdGrenEpi_)) - _EXP_(- _uLPARSurf_ /(_cLOptRefGren_ * _uFunTmProdGrenEpi_)))) +(1.0 - _UseSteeleGren_) *(1.0 /(_aExtCoefEpi_ * _uDepthWEpi_) * _LN_((1.0 + _uLPARSurf_ / (_hLRefGren_ * _uFunTmProdGrenEpi_)) / (1.0 + _aLPARBotEpi_ /(_hLRefGren_ * _uFunTmProdGrenEpi_))));
...
_oChla_ = _IF_ ( _aInclStrat_ _EQ_ _TRUE_ ) _THEN_ (_oChlaEpi_ * _uDepthWEpi_ + _oChlaHyp_ * _uDepthWHyp_ ) / _sDepthW_ _ELSE_ _oChlaEpi_ _ENDIF_;
_aLPIEpi_ =  (_uLPARSurf_- _aLPARBotEpiOpen_ + _uhLVeg_* _LN_((1.0 +_aLPARBotEpiOpen_/ _uhLVeg_)  /(1.0 + _uLPARSurf_  / _uhLVeg_) + _NearZero_  ) ) / _uDepthWEpi_;
...
_tDPrimTotT_ = _IF_ (_aInclStrat_ _EQ_ _FALSE_ ) _THEN_(_wDAssPhytEpi_ - _wDRespPhytWEpi_) * _uDepthWEpi_ - _tDRespPhytS_ _ELSE_ (_wDAssPhytEpi_ - _wDRespPhytWEpi_) * _uDepthWEpi_ + (_wDAssPhytHyp_ - _wDRespPhytWHyp_) * _uDepthWHyp_ - _tDRespPhytS_ _ENDIF_;
...
_tDConsFiJvPiscNut_ = _sDFiJv_ / _aDFish_ * _tDConsPiscNut_;
...
_vDredDepthW_ =_IF_ (_aInclStrat_ _EQ_ _TRUE_) _THEN_ 0.0 _ELSE_ _akDredDepth_ * _sDepthW_ _ENDIF_;
...
_aDRelTotT_ = _uDepthWHyp_ * _aRelDeltaWHyp_ * ( _oDDetWHyp_ + _oDIMWHyp_ + _oDDiatWHyp_ + _oDGrenWHyp_ + _oDBlueWHyp_ + _oDZooHyp_ ) + _fMarsh_ * _sDepthWM_ * _aRelDeltaWM_ * ( _oDDetWM_ + _oDIMWM_ + _oDDiatWM_ + _oDGrenWM_ + _oDBlueWM_ + _oDZooM_) + _uDepthWEpi_ * _aRelDeltaWEpi_ * (_oDDetWEpi_ + _oDIMWEpi_ + _oDDiatWEpi_ + _oDGrenWEpi_ + _oDBlueWEpi_ + _oDZooEpi_ );
...
_aSiRelTotT_ = _uDepthWHyp_ * _aRelDeltaWHyp_ * ( _oSiO2WHyp_ + _oSiDetWHyp_ + _cSiDDiat_*_oDDiatWHyp_) + _fMarsh_ * _sDepthWM_ * _aRelDeltaWM_ * ( _oSiO2WM_ + _oSiDetWM_ + _cSiDDiat_*_oDDiatWM_) + _uDepthWEpi_ * _aRelDeltaWEpi_ * ( _oSiO2WEpi_ + _oSiDetWEpi_ + _cSiDDiat_*_oDDiatWEpi_);
_aO2RelTotT_ = _uDepthWHyp_ * _aRelDeltaWHyp_* _oO2WHyp_ + _fMarsh_ * _sDepthWM_ * _aRelDeltaWM_ * _oO2WM_ + _uDepthWEpi_ * _aRelDeltaWEpi_ * _oO2WEpi_ ;
...
_aDTotT_ = _aDTotTHyp_ + (_oDIMWEpi_+_oDDetWEpi_+_oDDiatWEpi_+_oDGrenWEpi_+_oDBlueWEpi_+_oDZooEpi_) * _uDepthWEpi_ +_aDFish_+_sDPisc_+_aDVeg_ +_sDIMS_+_sDHumS_+_sDDetS_+_sDDiatS_+_sDGrenS_+_sDBlueS_+_sDBent_ + _aDTotM_;
_aNTotT_ = _aNTotTHyp_+ (_oNH4WEpi_+_oNO3WEpi_+_oNDetWEpi_+_oNDiatWEpi_+_oNGrenWEpi_+_oNBlueWEpi_ +_oNZooEpi_)*_uDepthWEpi_ +_aNFish_+_aNPisc_+_aNVeg_ +_sNH4S_+_sNO3S_+_sNDetS_+_sNHumS_+_sNDiatS_+_sNGrenS_+_sNBlueS_ +_sNBent_ + _aNTotM_;
_aPTotT_ = _aPTotTHyp_ + (_oPO4WEpi_+_oPDetWEpi_+_oPDiatWEpi_+_oPGrenWEpi_+_oPBlueWEpi_ +_oPZooEpi_+_oPAIMWEpi_) * _uDepthWEpi_ +_aPFish_+_aPPisc_+_aPVeg_ +_sPO4S_+_sPDetS_+_sPHumS_+_sPDiatS_+_sPGrenS_+_sPBlueS_ +_sPAIMS_+_sPBent_ + _aPTotM_;
_aSiTotT_ = _aSiTotTHyp_ + (_oSiO2WEpi_ + _oSiDetWEpi_ + _cSiDDiat_*_oDDiatWEpi_) *_uDepthWEpi_ + _sSiDetS_ + _cSiDDiat_*_sDDiatS_ + _aSiTotM_;
...
_cCheckVegHeight_ = _IF_ ( _aVegWin_ _EQ_ 1 _AND_  _uVegHeight_ _LT_ _MIN_ (_cVegHeightWin_, _sDepthW_) ) _THEN_ 1.0 _ELSEIF_ ( _aVegWin_ _EQ_ 1 _AND_  _uVegHeight_ _GT_ _MIN_ (_cVegHeightWin_, _sDepthW_) ) _THEN_ 2.0 _ELSEIF_ ( _aVegWin_ _EQ_ 0 _AND_  _uVegHeight_ _LT_ _MIN_ (_cVegHeight_, _sDepthW_) ) _THEN_  3.0 _ELSEIF_ ( _aVegWin_ _EQ_ 0 _AND_  _uVegHeight_ _GT_ _MIN_ (_cVegHeight_, _sDepthW_) ) _THEN_ 4.0 _ELSE_ 0.0 _ENDIF_;
